...
- **Stripe CLI 連携** - アプリ内から `stripe listen` を起動・停止し、Webhook イベントをリアルタイムにキャプチャ
...
- [Stripe CLI](https://docs.stripe.com/stripe-cli)（任意、リアルタイムイベント取得に必要）
...
| Stripe 通信 | reqwest（HTTP）、Stripe CLI（イベント） |
